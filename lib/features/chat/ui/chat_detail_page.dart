...
class _ChatDetailPageState extends State<ChatDetailPage> with WidgetsBindingObserver {
  ...
  Future<void> _loadFullHistory() async {
    ...
    try {
      ...
      final pending = _messages.where((m) => _pendingTempIds.contains(m.id)).toList();
      setState(() {
        _messages
          ..clear()
          ..addAll(allHistory);
      ...
      _showSnack(loc.loadHistoryError);
    }
  }
  ...
  void _onRoomEvent(mx.Event e) {
    if (e.type.startsWith('m.call.')) {
      MatrixService.forceSync(timeout: 0).then((_) => _mergeRecentFromSync());
      return;
    }
  ...
  Future<void> _sendMessage() async {
    ...
    if (eventId == null) {
      ...
      _showSnack(loc.sendMessageError);
      return;
    }
  ...
  Future<void> _attachFile() async {
    if (_uploadHandle != null || _isPickingFile) return;
    ...
    try {
      ...
      final result = await MatrixService.uploadWithProgress(
        name: name,
        data: bytes,
        mime: mime,
        onStart: (h) {
      ...
      if (result.handle.isCancelled) {
        _showSnack(loc.cancel);
        return;
      }
      if (result.mxc == null) {
        _showSnack(loc.sendFileError);
        return;
      }

      if (mime.startsWith('image/')) {
        await MatrixService.sendImageEvent(
        ...
        await MatrixService.sendFileEvent(
          roomId: widget.room.id,
          name: name,
          mxc: result.mxc!,
          mime: mime,
  ...
  Widget _buildDateSeparator(DateTime date) {
    ...
    final text = DateFormat.yMMMMd(localeName).format(date);
    return Padding(
  ...
  Widget _buildMessageBubble(Message msg, bool isMe, {required int index}) {
    ...
    final timeText = DateFormat('HH:mm, dd.MM.yyyy', localeName).format(msg.timestamp);
    ...
    final header = Row(
      ...
      children: [
        ...
        Text(timeText, style: const TextStyle(fontSize: 10, color: Colors.black45)),
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: Text(loc.audioCall),
                onTap: () {
                  Navigator.pop(context);
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) => OutgoingAudioCallPage(
                        roomId: widget.room.id,
                  ...
                  child: Row(
                    children: [
                      _uploadHandle != null
                        ...
                        child: Row(
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () => setState(() { _uploadHandle?.cancel(); }),
                            ),
                          ],
                        ),
                      )
                        ...
                        onPressed: _isPickingFile ? null : _attachFile,
                        tooltip: _isPickingFile ? null : AppLocalizations.of(context)!.attach,
                      ),
                      Expanded(
                        child: TextField(
  ...
  Widget _buildImageBubble(Message msg, bool isMe, Widget header) {
    ...
    if (url == null) {
      return _bubbleWrapper(
        isMe: isMe,
        header: header,
        child: Text(msg.text.isNotEmpty ? msg.text : loc.imagePlaceholder),
      );
    }
  ...
  Widget _buildFileBubble(Message msg, bool isMe, Widget header) {
    final loc = AppLocalizations.of(context)!;
    final name = msg.fileName ?? (msg.text.isEmpty ? loc.filePlaceholder : msg.text);
      ...
      child: Row(
        ...
        children: [
          if (!isMe)
            const Padding(
              padding: EdgeInsets.only(right: 8.0),
              child: CircleAvatar(radius: 12, child: Icon(Icons.person, size: 12)),
            ),
          Expanded(
            child: Column(
              crossAxisAlignment: isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start,
