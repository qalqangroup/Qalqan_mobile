...
class OutgoingCallService {
  ...
  Future<void> startCall({required String roomId}) async {
    _iceQueue.clear();
    try { await _onEventSub?.cancel(); } catch (_) {}
    _onEventSub = null;
    ...
    _onEventSub = _matrixClient!.onTimelineEvent.stream.listen((e) async {
      final String? evRoomId = (() {
        try { return e.room.id; } catch (_) {}
        try { return e.roomId; } catch (_) {}
        return null;
      })();
      if (_roomId != null && evRoomId != _roomId) return;
    ...
    _peerConnection!.onIceGatheringState = (RTCIceGatheringState s) async {
  ...
  Future<void> _finish({bool endCallkit = true}) async {
    ...
    if (_callId != null) {
      try { AuthDataCall.instance.outgoingCallIds.remove(_callId); } catch (_) {}
